...
// Force JSON content-type early (helps when Vercel overrides)
app.use((req, res, next) => {
  res.setHeader('Content-Type', 'application/json');
  next();
});

// Log every request (very useful in Vercel logs to see if route is hit)
app.use((req, res, next) => {
  console.log(`[${new Date().toISOString()}] ${req.method} ${req.url} - from ${req.headers['user-agent'] || 'unknown'}`);
  next();
});

// Quick health check endpoint - should always return 200 JSON
...
// Global error handler - strengthened
app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (!res.headersSent) {
    res.status(500).json({ 
      ...
      message: process.env.NODE_ENV === 'development' ? err.message : "Server error - check Vercel logs",
      // No full stack in prod
    });
  } else {
    console.warn("Headers already sent - cannot send JSON error");
  }
});
