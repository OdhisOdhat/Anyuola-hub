...
export default function About() {
  ...
  const [fullViewPhoto, setFullViewPhoto] = useState<GalleryPhoto | null>(null);

  // Official authentic documentary photo requested by user: Price1.jpeg
  const defaultBursaryPhoto: GalleryPhoto = {
  ...
  const heroPhoto = photos.find(p => !isPhotoDeleted(p) && p.homepage_section === "hero") || defaultBursaryPhoto;
  const bursaryPhoto = photos.find(p => !isPhotoDeleted(p) && (p.homepage_section === "bursary" || (!p.homepage_section && p.category === "bursary"))) || defaultBursaryPhoto;
  ...
  const rawFeatured = photos.filter(p => !isPhotoDeleted(p) && (p.show_on_homepage || p.homepage_section === "featured_carousel" || p.homepage_section === "bursary" || p.id === "photo-bursary-ceremony-1"));
  const featuredReelPhotos = rawFeatured.length > 0 ? rawFeatured : [defaultBursaryPhoto];
    ...
    <div className="space-y-24 pb-20">
      ...
      <section className="space-y-8 px-4">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          >
            <div 
              onClick={() => bursaryPhoto && setFullViewPhoto(bursaryPhoto)}
              className="relative aspect-[4/3] overflow-hidden bg-zinc-100 flex items-center justify-center cursor-pointer group/photo"
              title="Click to view full authentic photograph (No AI enhancement)"
            >
              {bursaryPhoto && !isPhotoDeleted(bursaryPhoto) ? (
                <>
                  <img 
                    ...
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute inset-0 bg-zinc-950/20 opacity-0 group-hover/photo:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                    <span className="px-3.5 py-1.5 rounded-xl bg-zinc-900/90 text-white text-xs font-bold flex items-center gap-1.5 shadow-lg backdrop-blur-sm">
                      <Eye className="w-3.5 h-3.5 text-emerald-400" />
                      View Authentic Photo
                    </span>
                  </div>
                </>
              ) : (
              ...
              <div className="absolute top-4 left-4 flex flex-wrap items-center gap-2">
                <span className="px-3 py-1.5 rounded-full bg-zinc-900/85 backdrop-blur-md text-emerald-400 text-[10px] font-black uppercase tracking-wider border border-white/10">
                  Education Committee
                </span>
                <span className="px-2.5 py-1 rounded-full bg-emerald-500 text-zinc-950 text-[9px] font-black uppercase tracking-wider shadow-sm">
                  Documentary Photo
                </span>
              </div>
              {isAdmin && (
            ...
            <div className="p-6 sm:p-8 space-y-3">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] font-black uppercase tracking-wider text-emerald-700">
                  North Kadem Community Action
                </span>
                <button
                  onClick={() => bursaryPhoto && setFullViewPhoto(bursaryPhoto)}
                  className="text-xs text-zinc-500 hover:text-emerald-700 font-bold flex items-center gap-1 cursor-pointer transition-colors"
                >
      ...
      {/* Lightbox Modal for Authentic Unenhanced Photos */}
      <AnimatePresence>
        {fullViewPhoto && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-4xl bg-zinc-950 rounded-3xl overflow-hidden border border-zinc-800 shadow-2xl z-10 flex flex-col max-h-[92vh]"
            >
              <div className="p-4 sm:p-5 flex items-center justify-between border-b border-zinc-800/80 bg-zinc-900/60">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs text-zinc-400 font-medium">Original • No AI Enhancement</span>
                </div>
                <button
                  onClick={() => setFullViewPhoto(null)}
                  className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl transition-all cursor-pointer"
              ...
              <div className="flex-1 overflow-auto bg-black flex items-center justify-center p-2 sm:p-4">
                <img
                  ...
                  alt={fullViewPhoto.caption || fullViewPhoto.title}
                  className="max-h-[65vh] w-auto max-w-full object-contain rounded-xl shadow-xl"
                  referrerPolicy="no-referrer"
                />
              </div>

              <div className="p-5 sm:p-6 bg-zinc-950 border-t border-zinc-800/80 space-y-2 text-left">
                <div className="flex flex-wrap items-center gap-3">
                  <h3 className="text-base sm:text-lg font-black text-white">
                    {fullViewPhoto.title || "Documentary Photograph"}
                  </h3>
                  <span className="text-xs text-emerald-400 font-bold px-2.5 py-0.5 rounded-full bg-emerald-500/10">
                    {fullViewPhoto.location || "North Kadem, Kenya"}
                  </span>
                  {fullViewPhoto.date && (
                ...
                <p className="text-xs sm:text-sm text-zinc-300 font-medium leading-relaxed">
                  {fullViewPhoto.description || fullViewPhoto.caption}
                </p>
              </div>
            </motion.div>
          </div>
